...
class FieldEasyDrawable(private val context: Context,private val screenUnit:Double):Drawable() {
    ...
    private val b = Point((a.x+3*screenUnit).toInt(),a.y)
    private val c = Point((b.x),(b.y-screenUnit).toInt())
    private val d = Point((c.x+2*screenUnit).toInt(),(c.y))
    private val e = Point((d.x),(d.y+screenUnit).toInt())
    private val f = Point((e.x+3*screenUnit).toInt(),(e.y))
    private val g = Point((f.x),(f.y+10*screenUnit).toInt())
    private val h = Point((g.x-3*screenUnit).toInt(), (g.y))
    private val i = Point((h.x).toInt(), (h.y+screenUnit).toInt())
    private val j = Point((i.x-2*screenUnit).toInt(), (i.y).toInt())
    private val k = Point((j.x).toInt(), (j.y-screenUnit).toInt())
    private val l = Point((k.x-3*screenUnit).toInt(), (k.y).toInt())
    ...
    override fun draw(canvas: Canvas) {
        ...
        canvas.drawCircle((screenUnit*5).toFloat(), (screenUnit*7).toFloat(), (screenUnit/20).toFloat(),paint)
