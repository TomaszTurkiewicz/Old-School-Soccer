...
class TemporaryActivity : AppCompatActivityCoroutine() {
                        ...
                        if(user!=null){
                            ...
                            if(!databaseCreated){
                                ...
                                launch {
                                    ...
                                    Toast.makeText(this@TemporaryActivity,"DATABASE CREATED", Toast.LENGTH_LONG).show()
                                    Functions.databaseCreatedToSharedPreferences(this@TemporaryActivity,true,user.uid)
                                }
                            }else{
                                Toast.makeText(this@TemporaryActivity,"DATABASE ALREADY EXISTS", Toast.LENGTH_LONG).show()
                            }
                        }
                    }else{
                        updateLoginUI()
        ...
        dbRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(!snapshot.exists()){
                    ...
                    val shpUser = Functions.readUserFromSharedPreferences(this@TemporaryActivity,user.uid)
                    if(fUser!=null){
                        ...
                        Functions.saveUserToSharedPreferences(this@TemporaryActivity,shpUser)
                        Functions.saveLoggedStateToSharedPreferences(this@TemporaryActivity, loggedInStatus = LoggedInStatus(true,user.uid))
                        updateLoginUI()
                    }
    ...
    private fun makeConstraintLayout() {
        ...
        set.clone(temporary_layout)

        set.connect(login_logout_Image_view.id, ConstraintSet.TOP,temporary_layout.id, ConstraintSet.TOP,screenUnit)
        set.connect(login_logout_Image_view.id, ConstraintSet.RIGHT,temporary_layout.id, ConstraintSet.RIGHT,marginLeft)

        set.connect(user_name.id, ConstraintSet.TOP,temporary_layout.id, ConstraintSet.TOP,screenUnit)
        set.connect(user_name.id, ConstraintSet.LEFT,temporary_layout.id, ConstraintSet.LEFT,marginLeft)
        ...
        set.connect(choose_game_type_button_main_activity.id, ConstraintSet.LEFT,temporary_layout.id, ConstraintSet.LEFT,marginLeft)

        set.applyTo(temporary_layout)
    }

    private fun setBackgroundGrid() {
        temporary_layout.background = TileDrawable((ContextCompat.getDrawable(this, R.drawable.background)!!), Shader.TileMode.REPEAT,screenUnit)
